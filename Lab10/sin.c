...
void sin_speed_x87(const char* descr, void sin_x87(double*, double*, uint64_t), uint64_t length) {
    double* input = create_array(length);
    assert(input != NULL);
    double* output = (double*)malloc(length * sizeof(double));
    assert(output != NULL);

    TIMING_RESULT(descr, sin_x87(input, output, length));

    free(input);
    free(output);
}

void sin_speed_stdlib(const char* descr, void sin_stdlib(double*, double*, uint64_t), uint64_t length) {
    double* input = create_array(length);
    assert(input != NULL);
    double* output = (double*)malloc(length * sizeof(double));
    assert(output != NULL);

    TIMING_RESULT(descr, sin_stdlib(input, output, length));
...
int main(void) {
    ...
    sin_speed_x87("sin_x87", sin_x87, POLYNOMIAL_LENGTH);
    sin_speed_stdlib("sin_stdlib", sin_stdlib, POLYNOMIAL_LENGTH);
